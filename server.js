...
function resolveDbPath() {
  ...
  const candidates = [
    '/data/sunloc.db',              // Railway persistent volume (if configured)
    '/tmp/sunloc.db',               // Railway tmp — always writable ✅
    path.join(__dirname, 'sunloc.db') // Local development
  ];
  ...
  for (const dbPath of candidates) {
    try {
      ...
      console.log(`[DB] Path not writable: ${dbPath}, trying next...`);
    }
  }

  console.log(`[DB] Using fallback: /tmp/sunloc.db`);
  return '/tmp/sunloc.db';
}

const DB_PATH = resolveDbPath();
const db = new Database(DB_PATH);
console.log(`[DB] Database opened successfully at: ${DB_PATH}`);
...
// If DATABASE_URL exists (Railway PostgreSQL), sync data to SQLite on startup
// This ensures your existing data is preserved when switching to new server
async function migrateFromPostgres() {
  ...
  try {
    ...
    console.log('[Migration] PostgreSQL detected — syncing existing data to SQLite...');
    
    // Check if we already have data in SQLite
    const existingPlanning = db.prepare('SELECT COUNT(*) as count FROM planning_state').get();
    ...
    // Migrate planning_state
    try {
      const pgPlanning = await pgPool.query('SELECT * FROM planning_state ORDER BY id DESC LIMIT 1');
      if (pgPlanning.rows.length > 0) {
        const row = pgPlanning.rows[0];
        db.prepare('INSERT OR REPLACE INTO planning_state (id, state_json, saved_at) VALUES (?, ?, ?)').run(1, row.state_json, row.saved_at || new Date().toISOString());
        console.log('[Migration] ✅ Planning state migrated');
      }
    } catch(e) { console.log('[Migration] Planning state skip:', e.message); }

    // Migrate DPR records
    try {
      const pgDpr = await pgPool.query('SELECT * FROM dpr_records');
      const insertDpr = db.prepare('INSERT OR IGNORE INTO dpr_records (floor, date, data_json, saved_at) VALUES (?, ?, ?, ?)');
      pgDpr.rows.forEach(r => insertDpr.run(r.floor, r.date, r.data_json, r.saved_at || new Date().toISOString()));
      console.log(`[Migration] ✅ DPR records migrated: ${pgDpr.rows.length}`);
    } catch(e) { console.log('[Migration] DPR skip:', e.message); }

    // Migrate tracking labels
    try {
      const pgLabels = await pgPool.query('SELECT * FROM tracking_labels');
      const insertLabel = db.prepare(`INSERT OR IGNORE INTO tracking_labels 
        (id, batch_number, label_number, size, qty, customer, colour, pc_code, po_number, machine_id, generated, printed, printed_at, voided, void_reason, voided_by, qr_data)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`);
      pgLabels.rows.forEach(r => insertLabel.run(
        r.id, r.batch_number||r.batchNumber, r.label_number||r.labelNumber, r.size, r.qty,
        r.customer||'', r.colour||'', r.pc_code||r.pcCode||'', r.po_number||r.poNumber||'',
        r.machine_id||r.machineId||'', r.generated||new Date().toISOString(),
        r.printed?1:0, r.printed_at||null, r.voided?1:0, r.void_reason||'', r.voided_by||'',
        r.qr_data||r.qrData||''
      ));
      console.log(`[Migration] ✅ Labels migrated: ${pgLabels.rows.length}`);
    } catch(e) { console.log('[Migration] Labels skip:', e.message); }

    // Migrate tracking scans
    try {
      const pgScans = await pgPool.query('SELECT * FROM tracking_scans');
      const insertScan = db.prepare(`INSERT OR IGNORE INTO tracking_scans 
        (id, label_id, batch_number, dept, type, ts, operator, size, qty)
        VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)`);
      pgScans.rows.forEach(r => insertScan.run(
        r.id, r.label_id||r.labelId, r.batch_number||r.batchNumber,
        r.dept, r.type, r.ts||new Date().toISOString(),
        r.operator||'', r.size||'', r.qty||0
      ));
      console.log(`[Migration] ✅ Scans migrated: ${pgScans.rows.length}`);
    } catch(e) { console.log('[Migration] Scans skip:', e.message); }

    await pgPool.end();
    console.log('[Migration] ✅ Migration complete — all data preserved!');
  } catch(e) {
    console.log('[Migration] PostgreSQL not available or migration skipped:', e.message);
  }
}

// Run migration after server starts
setTimeout(migrateFromPostgres, 2000);
